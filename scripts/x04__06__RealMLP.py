...
class F01__RealMLP:
    ...
    def fit(self, X, y, ):
        ...
        X_resampled = self.data_convs(X_resampled, is_emb=self.is_conv_emb, is_log_conv=self.is_conv_log, is_train=True)
        X_scaled = self.scaler.fit_transform(X_resampled)
        self.model.fit(X_scaled, y_resampled)

    def predict(self, X):
        X_nu = X.select_dtypes("float")
        X_nu = self.data_convs(X_nu, is_emb=self.is_conv_emb, is_log_conv=self.is_conv_log, is_train=True)
        X_scaled = self.scaler.transform(X_nu)
